...
huskyGames2016.filter(huskiesLost).forEach(function(temp, index, arr) {
  console.log(arr[index].opponent + " at " + arr[index].home + ", " + arr[index].opponent_score + " to " + arr[index].home_score);
});
...
console.log(huskyGames2016.filter(function(arr) {
    return arr.fumbles >= 1;
}).length);
...
function mostYardsPassing(game1, game2) {
  if (!('passing_yards' in game1) || game2.passing_yards > game1.passing_yards) {
    return game2;
  }
  return game1;
}
